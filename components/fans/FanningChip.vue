<!-- FanningChip.vue -->
<template>
  <NuxtLink :to="`/${user.username}/following`" class="text-decoration-none">
    <div class="social-stat-chip" :class="sizeClass">
      <span class="stat-label">Following</span>
      <span class="stat-count">{{ fanningCount }}</span>
    </div>
  </NuxtLink>
</template>
...
const fanningCount = ref(0)

const fetchFanningCount = async () => {
  try {
    const count = await userFansStore.fetchFanningCount(props.user.username)
    fanningCount.value = count
  } catch (error) {
